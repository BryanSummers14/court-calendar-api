...
// Parsers
const MidvaleJusticeCase = require('./parsers/midvale-justice.parser');
...
const run = async (_calendar, _type = 'DISTRICT') => {
  ...
  parser.on('pdfParser_dataReady', pdfData => {
    ...
    for (let i = 0; i < allPages.length; i++) {
      ...
      Page['Texts'] = Page['Texts'].map(_TEXT => {
        return _TEXT['R'].map(_R => {
          ...
          _R['T'] = _R['T'].replace(/%3A/g, ':');
          _R['T'] = _R['T'].replace(/%2C/g, ',');
          _R['T'] = _R['T'].replace(/%2F/g, '-');
          pageLines.push(_R['T']);
          return _R;
        });
      });
    ...
    for (let j = 0; j < formattedPages.length; j++) {
      ...
      pg.PageText = pg['Texts'].map(_text => _text[0]['T'].trim());
      const indices = [];
